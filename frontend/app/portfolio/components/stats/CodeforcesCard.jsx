...
const CodeforcesCard = ({
  ...
  const LoadingSkeleton = () => (
    <div className="loading-skeleton h-16 rounded-xl bg-navy-600/30"></div>
  );
  ...
  const getRatingInfo = (rating) => {
    ...
    if (rating >= 3000)
      return { color: "#aa0000", title: "Legendary Grandmaster" };
    if (rating >= 2600)
      return { color: "#ff0000", title: "International Grandmaster" };
    if (rating >= 2400) return { color: "#ff8c00", title: "Grandmaster" };
    if (rating >= 2300)
      return { color: "#ffcc00", title: "International Master" };
  ...
  return (
    <div
      className="platform-banner rounded-2xl p-8 md:p-10 hover:scale-105 transition-all duration-500 group border-2"
      style={{
        background:
          "linear-gradient(135deg, rgba(55,0,60,0.4), rgba(30,0,50,0.3))",
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-5 space-x-4 mb-4">
              ...
              <div>
                ...
                >
                  <h3
                    className="text-xl font-bold group-hover:text-yellow-300 transition-all duration-300 cursor-pointer"
                    style={{ color: currentRatingInfo.color }}
                  >
                    {data.username}{" "}
                    <span className="text-xl ml-2 opacity-70 group-hover:opacity-100">
                      🔗
                    </span>
                  </h3>
                </a>
            ...
            {/* Stats */}
            <div className="flex justify-center items-end flex-col gap-4  mb-4">
              <div className="flex items-center gap-2 bg-navy-600/40 rounded-xl">
                <div className="flex items-center gap-2">
                  <FaStar
                    className=""
                    style={{ color: currentRatingInfo.color }}
                  />
                  <span className="text-gray-300 mb-0">Contest Rating</span>
                </div>
                :
                <span
                  className="font-bold"
              ...
              <div className="flex items-center gap-2 bg-navy-600/40 rounded-xl">
                <div className="flex items-center mb-0 gap-2">
                  <FaChartLine
                    className=""
                    style={{ color: maxRatingInfo.color }}
                  />
